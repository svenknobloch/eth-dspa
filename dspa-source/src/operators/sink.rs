    ...
    D: StreamRecord,
{
    fn publish(&self, ctx: &Context) {
        ...
        socket.connect(&format!("ipc://{}", SOURCE_SOCKET)).expect("Failed to connect!");
        ...
        self.sink(Pipeline, "Publish", move |input| {
            input.for_each(|cap, data| {
                ...
                vec.drain(..).for_each(|record| {
                    println!("{} record {{ time: {}, timestamp: {}, id: {:?}}} sent!", D::TOPIC.to_string(), *cap.time(), record.timestamp(), record.id());
